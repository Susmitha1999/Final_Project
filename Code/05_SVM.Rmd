...
training_model <- training %>%
  select(
    ...
    srad_mean, vp_mean, heat_stress_days,
    soilpH, om_pct, soilk_ppm, soilp_ppm,
    elevation, longitude, latitude,
    previous_crop, doy_planted, doy_harvested, season_length,
    year
  ) %>%
  mutate(
...
set.seed(931)
yield_split <- initial_split(training, prop = 0.70, strata = yield_mg_ha)
...
```{r}
# 04_svm_model.R
# SVM with RBF kernel for corn yield prediction
# 70/30 stratified split by yield, 10-fold CV, regular grid search
# Designed for HPC cluster (Sapelo2) — no subsampling
...
# ── Fit final model on full train_data ───────────────────────────────────────
...
# ── Save model, metrics, tuning results ───────────────────────────────────────
